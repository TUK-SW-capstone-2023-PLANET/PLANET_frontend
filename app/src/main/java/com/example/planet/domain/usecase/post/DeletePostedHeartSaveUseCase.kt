...
class DeletePostedHeartSaveUseCase @Inject constructor(private val postingRepository: PostingRepository) {
    suspend operator fun invoke(postId: PostId): Flow<ApiState> {
        return postingRepository.deletePostedHeartSave(postId)
    }
}