...
class DeleteCommentHeartUseCase @Inject constructor(private val postingRepository: PostingRepository) {
    suspend operator fun invoke(commentId: CommentId): Flow<ApiState> {
        return postingRepository.deleteCommentHeart(commentId)
    }
}